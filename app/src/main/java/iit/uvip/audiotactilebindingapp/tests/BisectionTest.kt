...
class BisectionTest(ctx: Context, data: TestData, private val mImageView:ImageView) : Test(ctx, data)
{
    ...
    override fun show(trialid:Int, isRepeat:Boolean){
        ...
        mStimuliHandler.postDelayed({
            deliverStimulus(mTrial as BisectionTrial, TRIAL_STAGE_1)
            testEvent.accept(EVENT_STIMULI_START)
        }, FIRST_STIMULUS_DELAY.toLong())

        mStimuliHandler.postDelayed({
            deliverStimulus(mTrial as BisectionTrial, TRIAL_STAGE_2)
        }, ((mTrial as BisectionTrial).position + FIRST_STIMULUS_DELAY).toLong())

        mStimuliHandler.postDelayed({
            deliverStimulus(mTrial as BisectionTrial, TRIAL_STAGE_3)
        }, (BisectionTrial.LAST_STIMULUS_DELAY + FIRST_STIMULUS_DELAY).toLong())
    ...
    private fun deliverAVStimuli(trial:BisectionTrial, stage:Int=0){
