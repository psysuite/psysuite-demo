...
import org.albaspazio.psysuite.network.ApiClient
...
class ResultsManager private constructor(private var activity: Activity) {
    ...
    private fun sendByEmail(result: TestResult) {
        mailJob = GlobalScope.launch {
            try {
//                MailIntent.composeEmail(activity, "org.albaspazio.psysuite.provider", emailRecipients, result.mailsubject, result.mailbody, result.res_files)
                    ...
                    show2ChoisesDialog(activity, resources.getString(R.string.warning),
                        resources.getString(R.string.ask_send_results_intent), resources.getString(R.string.yes), resources.getString(R.string.no),
                        {
                            MailIntent.composeEmail(
                                activity,
                                "org.albaspazio.psysuite.provider",
                                emailRecipients,
                                result.mailsubject,
                                result.mailbody,
                                result.res_files
    ...
    fun validateIntegrationFixes(): Boolean {
        ...
        try {
            val testJson = JSONObject().apply {
                ...
                put("classes", JSONArray().apply { put("org.albaspazio.psysuite.tests.TestBIS") })
            }
        ...
        try {
            val fullClassName = "org.albaspazio.psysuite.tests.TestBIS"
